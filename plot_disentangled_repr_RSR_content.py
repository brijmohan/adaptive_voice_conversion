...
class Inferencer(object):
    ...
    def inference_from_path(self):
        ...
        for mdir in male_dirs:
            ...
            spk_feats = []
            for utt in utts:
                utt_feat = self.inference_one_utterance_content(utt)
        ...
        cmap = get_cmap(MAX_UTT_PER_SPK, name='tab10')
        start, end = 0, None
        for i, (spk, featlen) in enumerate(spk2featlen.items()):
            end = start + featlen
            if spk2gender[spk] == 'm':
                ...
                col = 'b'
                mark = '^'
            ax1.scatter(Y[start:end, 0], Y[start:end, 1], c=cmap.colors, marker=mark)
            start = end
